...
import { Plus, Clock, MoreVertical, LayoutGrid, Sparkles, Zap, Copy, FileText } from "lucide-react";
...
const BoardsView = ({ isDark = false, boards = [], docs = [], fetchBoards, handleCreateDoc, setShowModal }) => {
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-10">
        ...
        {(docs && docs.length > 0) ? (
          <section className="mt-10">
            <h2 className={`text-xs font-bold uppercase tracking-widest mb-4 ${isDark ? "text-slate-500" : "text-slate-400"}`}>Your Documents</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {/* Create New Doc Card */}
              ...
              {docs.map((doc, i) => (
                <motion.div key={doc.ID} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
                  ...
                  className={`rounded-2xl p-5 border cursor-pointer transition-all shadow-sm hover:shadow-xl ${isDark ? "bg-slate-800 border-slate-700 hover:border-emerald-500" : "bg-white border-slate-200 hover:border-emerald-300"}`}>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className={`font-bold text-base truncate w-36 ${isDark ? "text-white" : "text-slate-800"}`}>{doc.Title}</h3>
                      <div className={`flex items-center gap-1 text-xs mt-1 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                        <Clock size={11} />
