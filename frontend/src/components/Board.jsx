...
const Board = () => {
  ...
  const saveToBackend = useCallback(async () => {
    ...
    try {
      await axios.post(`https://reechomedia.com/api/boards/${id}/sync`,
        { 
          title: boardTitle, 
  ...
  useEffect(() => {
    const fetchBoardAndTasks = async () => {
      try {
        ...
        // Fetch Board
        const res = await axios.get(`https://reechomedia.com/api/boards/${id}?t=${Date.now()}`, { headers });
        ...
        if (role === "admin" || role === "member") {
          const [tasksRes, docsRes] = await Promise.all([
            axios.get(`https://reechomedia.com/api/tasks`, { headers }),
            axios.get(`https://reechomedia.com/api/docs`, { headers })
          ]);
  ...
  const handleDelete = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`https://reechomedia.com/api/boards/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      navigate("/dashboard");
  ...
  const handleSubmitReview = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.post(`https://reechomedia.com/api/boards/${id}/submit-review`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setReviewSent(true);
  ...
  const handleApprove = async () => {
    const token = localStorage.getItem("token");
    try {
      await axios.post(`https://reechomedia.com/api/boards/${id}/approve`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
